...
class StudyLoader:
    ...
    def load_study_remote(self, study_model, progress_callback=None, completion_callback=None):
        """
    ...
    def _download_study_worker(self, study_model, progress_callback):
        """Background worker that streams DICOM files natively and verifies them."""
        try:
            logger.info("Worker started. Phase 1: Download")
            ...
            session_main = _orthanc_http_session()
            
            logger.info(f"Looking up Orthanc internal ID for {study_uid}...")
            orthanc_id = orthanc.lookup_study_id(study_uid)
            if not orthanc_id:
                logger.error("Could not find study in Orthanc.")
                return None

            url = f"{orthanc.base_url}/studies/{orthanc_id}/instances"
            resp = session_main.get(url, timeout=30)
            ...
            if progress_callback:
                progress_callback(10, f"Streaming {total_instances} instances from Orthanc...")
            ...
            def download_task(inst):
                inst_id = inst["ID"]
                output_path = os.path.join(cache_dir, f"{inst_id}.dcm")
                if os.path.exists(output_path):
                    return True
                sess = _orthanc_http_session()
                file_url = f"{orthanc.base_url}/instances/{inst_id}/file"
                with sess.get(file_url, stream=True, timeout=req_timeout) as r:
                    r.raise_for_status()
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {executor.submit(download_task, inst): inst for inst in instances}
            ...
            for inst in instances:
                output_path = os.path.join(cache_dir, f"{inst['ID']}.dcm")
